...
function activate(context) {
  ...
  let consoleLog = vscode.commands.registerCommand(
    "extension.clog",
    function () {
      doLog(false);
    }
  ...
  let consoleLogBefore = vscode.commands.registerCommand(
    "extension.clogBefore",
    function () {
      doLog(true);
    }
  );
  ...
  function doLog(before) {
    ...
    if (supportedLangs.includes(lang)) {
      ...
      let log =
        startSpace +
        `debugPrint("${textEsc} \${${consoleValue}}", wrapWidth: 1024);`;
